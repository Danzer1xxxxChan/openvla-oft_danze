...
class FinetuneConfig:
    ...
    val_freq: int = 50                           # (When `use_val_set==True`) Validation set logging frequency in steps
...
def run_forward_pass(
    ...
    if not (use_l1_regression or use_diffusion):
        ...
        print("Ground truth token IDs:", (32000-ground_truth_token_ids[0, -169:-1]).tolist(), "\nPredicted token IDs:", (32000-predicted_token_ids[0]).tolist())
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # Decay the learning rate every `cfg.num_steps_before_decay` steps by a factor of 0.1.
    # Use StepLR so the decay repeats every `step_size` steps rather than occurring once.
    scheduler = StepLR(
        optimizer,
        step_size=cfg.num_steps_before_decay,  # Number of steps between LR decays
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            print(f"Rank {torch.distributed.get_rank()} finished dataloader normally")
