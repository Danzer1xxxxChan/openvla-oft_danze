...
import os
import shutil
from prismatic.vla.datasets.datasets import crop_right_square
import time
...
def prepare_images_for_vla(images: List[np.ndarray], cfg: Any) -> List[Image.Image]:
    ...
    for image in images:
        ...
        if image.shape != (OPENVLA_IMAGE_SIZE, OPENVLA_IMAGE_SIZE, 3):
            if cfg.showlab_franka_arm:
                print("Using cropping right")
...
def get_vla_action(
    ...
    with torch.inference_mode():
        ...
        if action_head is None:
            ...
            action, _ = vla.predict_action(**inputs, unnorm_key=cfg.unnorm_key, do_sample=False,proprio=proprio, proprio_projector=proprio_projector,use_film=use_film)
        else:
...
def get_vla_action_with_sampling(
    cfg: Any,
    vla: torch.nn.Module,
    processor: Any,
    obs: Dict[str, Any],
    ...
    with torch.inference_mode():
        ...
        # if action_head is None:
            # Standard VLA output (single-image inputs, discrete actions)
        action_list, _ = vla.predict_action_do_sample(
            **inputs,
            unnorm_key=cfg.unnorm_key,
            proprio=proprio,
            proprio_projector=proprio_projector,
