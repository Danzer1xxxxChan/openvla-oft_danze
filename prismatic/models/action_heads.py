...
class DiscreteActionHead(nn.Module):
    ...
    def __init__(
        ...
        self.action_dim = action_dim
        self.model = MLPResNet(
            num_blocks=2, input_dim=input_dim*ACTION_DIM, hidden_dim=hidden_dim, output_dim=action_dim*ACTION_DIM
        )
    ...
    def predict_action(self, actions_hidden_states):
        ...
        device = actions_hidden_states.device
        rearranged_actions_hidden_states = actions_hidden_states.reshape(batch_size, NUM_ACTIONS_CHUNK, -1)
        action = self.model(rearranged_actions_hidden_states) # (batch_size, chunk_len, 7*256)
        action = action.reshape(batch_size,NUM_ACTIONS_CHUNK*ACTION_DIM, -1)  # (batch_size, chunk_len*7 , 256)
        return action
#test end
