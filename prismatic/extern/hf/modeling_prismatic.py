...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def _discrete_prediction_do_sample(
        self,
        input_embeddings,
        all_actions_mask,
        projected_patch_embeddings,
        ...
        if action_head is not None:
            ...
            if temperature == 0:
                ...
                for _ in range(BEST_OF_N):
                    predicted_action_token_ids = (torch.multinomial(
                        probs.view(-1, probs.size(-1)), num_samples=1
                    ).view(probs.shape[:-1]).cpu().numpy())
                    ...
                    sample_action_list.append(normalized_actions)

                    return sample_action_list, actions_hidden_states

        return normalized_actions, actions_hidden_states

    def predict_action_do_sample(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        unnorm_key: Optional[str] = None,
        proprio=None,
        ...
        **kwargs: str,
        ...
        if use_diffusion:
            ...
            # Run regression or discrete token-based prediction
            normalized_actions, actions_hidden_states = self._discrete_prediction_do_sample(
                input_embeddings,
                all_actions_mask,
                projected_patch_embeddings,
                attention_mask,
        ...
        #print(f"normalized_actions, {normalized_actions}")
        # Unnormalize predicted actions
        actions = [self._unnormalize_actions(x, unnorm_key) for x in normalized_actions]
